...
<script>
    ...
    // ========== SCANNER INICIO ==========
    // Variável global para o scanner
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (btnScanner) {
            btnScanner.addEventListener('click', function() {
                // Se já estiver rodando, para. Se não, começa.
        ...
        function iniciarScanner() {
            ...
            statusDiv.innerText = "Iniciando câmera...";
            ...
            html5QrcodeScanner.start(
                ...
                statusDiv.innerText = "Erro: Permissão de câmera negada ou HTTPS ausente.";
                alert("Erro: O navegador bloqueou a câmera. Verifique se você está usando HTTPS.");
            });
        }
        ...
        function onScanSuccess(decodedText, decodedResult) {
            ...
            // Feedback visual
            statusDiv.className = "bg-success text-white text-center p-2 rounded mb-3";
            statusDiv.innerText = "Lido: " + decodedText;

            // Para o scanner após ler
            pararScanner();

            // Verifica se deve fazer auditoria rápida automática
            const localidadeSelecionada = selectLocalidade.value;
            
            if (localidadeSelecionada !== "") {
                // Se tiver localidade selecionada na busca rápida, executa auditoria AJAX
                realizarAuditoriaRapida(decodedText, localidadeSelecionada);
            } else {
                // Se não, submete o formulário de busca normal
                document.getElementById('formBusca').submit();
            }
        }

        function onScanFailure(error) {
            // Ignora erros de frame (muito comuns enquanto procura o código)
            // console.warn(`Code scan error = ${error}`);
        }

        // Função para Auditoria Rápida via AJAX (conecta com seu PHP)
        function realizarAuditoriaRapida(patrimonio, localidade) {
            ...
            formData.append('nova_localidade', localidade);

            statusDiv.style.display = 'block';
            statusDiv.className = "bg-warning text-dark text-center p-2 rounded mb-3";
            statusDiv.innerText = "Processando auditoria...";
            ...
            .then(data => {
                if (data.success) {
                    ...
                    inputPatrimonio.value = ''; // Limpa para o próximo
                    
                    // Recarrega a página após 1.5s para atualizar estatísticas
        ...
        // Vincula o botão "Auditoria Rápida" manual também
        ...
        if(btnRapidaManual) {
            btnRapidaManual.addEventListener('click', function() {
                ...
                if(!pat) { alert('Digite o patrimônio'); return; }
